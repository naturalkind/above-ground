...
#                    cursor_msg = 'Down Key - pitch(-):{}'.format(CMDS['pitch'])
                ...
                if start_fly:
                    if ixx > 200:
                        pid_output_throttle = pid_throttle.update(board.SENSOR_DATA['altitude'], 1.0)
                ...
                screen.clrtoeol()     
#                screen.addstr(7, 100, "Gyroscope: {}".format(board.SENSOR_DATA['gyroscope']))
#                screen.clrtoeol()    
#                screen.addstr(8, 100, "Accelerometer: {}".format(board.SENSOR_DATA['accelerometer']))
#                screen.clrtoeol()                                                  
...
#        curses_process = multiprocessing.Process(target=run_curses)

#        # Запускаем процесс с функцией, использующей curses
#        curses_process.start()

#        # Делаем что-то в главном процессе
#        # Ждем завершения процесса с использованием curses
#        curses_process.join()    
