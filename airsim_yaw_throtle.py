...
def test_pid_up(tracker_arg, dict_):
    ...
    client.confirmConnection()
    client.reset()
    client.enableApiControl(False, "BP_FlyingPawn2_2") # 
    ...
    # moveByManualAsync
    print (all_drone, client.isApiControlEnabled(vehicle_name=all_drone[0]))
...
#    f_kalman = KalmanFilterPID(Kp, Ki, Kd, 0.1)
    ...
    while True:
        ...
        current_height = client.getMultirotorState(vehicle_name=all_drone[0]).kinematics_estimated.position.z_val
    ...
    #    pid_output = f_kalman.update(target_height, current_height)
        ...
        client.moveByRC(vehicle_name=all_drone[0], rcdata = airsim.RCData(#pitch = dict_["pitch"], # 5.0 max
                                                                           throttle = -pid_output,
                                                                           #yaw=0.0,
                                                                           #roll=0.0,
                                                                           is_initialized = True,
...
#                                                       is_valid = True))
                ...
                print (X, Y, Z, dict_["throttle"], dict_["pitch"], control_signal_x)
                #print (f"Вращение по Z: {Z},  угол к обьекту: {angle_to_obj}, высота: {dict_['current_height']}")
