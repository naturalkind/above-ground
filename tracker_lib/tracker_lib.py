...
class TrackerLib(object):
    def __init__(self):
        ...
        self.trackers = {}
        self.tracker_weights = {'csrt': 0.6, 'kcf': 0.4, 'nanotrack': 0.1}
        # load config
        cfg.merge_from_file(args.config)

#        # load_weight
        self.setup_logging()
        self.silent_create_nanotracker()
        
    def setup_logging(self):
        # Настройка логгера
        self.logger = logging.getLogger('NanoTracker')
        self.logger.setLevel(logging.ERROR)  # Устанавливаем уровень логирования ERROR или выше

        # Опционально: добавляем обработчик для записи в файл
        file_handler = logging.FileHandler('nanotracker.log')
        file_handler.setLevel(logging.DEBUG)
        self.logger.addHandler(file_handler)

    def silent_create_nanotracker(self):
        ...
        try:
            ...
            self.logger.error(f"Error creating NnoTracker_RKNNLite: {str(e)}")
        finally:
            self.logger.removeHandler(ch)

        # Если нужно, можно получить и обработать захваченный вывод
        log_contents = log_capture_string.getvalue()
        if log_contents:
            self.logger.debug(f"Captured output during NanoTracker creation: {log_contents}")
    ...
    def init_tracker(self, img, bbox):
        ...
        self.trackers['nanotrack'] = self.NanoTracker
        self.trackers['nanotrack'].init(img, bbox)
        self.init_switch = True
