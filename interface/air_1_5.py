...
class YawPidController(PIDController):
    def __init__(self, kp, ki, kd, integral_limit=50, output_limit=80):
        super().__init__(kp, ki, kd, integral_limit)
        self.output_limit = output_limit
    
    def update(self, current_value, target_value, dt):
        # Плавное ограничение производной
        error = target_value - current_value
        
        # Фильтр для ошибки (убираем шум)
        filtered_error = self.smooth_filter(error)
        
        # Адаптивный P-коэффициент в зависимости от ошибки
        adaptive_kp = self.kp
        if abs(error) < 10:  # Малая ошибка
            adaptive_kp = self.kp * 0.3  # Меньшая реакция
        elif abs(error) > 50:  # Большая ошибка
            adaptive_kp = self.kp * 1.2  # Большая реакция
        
        self.integral += filtered_error * dt
        self.integral = max(-self.integral_limit, min(self.integral_limit, self.integral))
        
        derivative = (filtered_error - self.prev_error) / dt if dt > 0 else 0
        # Ограничение производной для устранения рывков
        derivative = max(-100, min(100, derivative))
        
        output = (adaptive_kp * filtered_error) + (self.ki * self.integral) + (self.kd * derivative)
        output = max(-self.output_limit, min(self.output_limit, output))
        
        self.prev_error = filtered_error
        return output
    
    def smooth_filter(self, value, alpha=0.7):
        """Экспоненциальное сглаживание"""
        if not hasattr(self, 'filtered_value'):
            self.filtered_value = value
        self.filtered_value = alpha * self.filtered_value + (1 - alpha) * value
...
class CombinedFPVSystem:
    def __init__(self, screen_width=1800, screen_height=1000):
        ...
        pygame.display.set_caption("ELRS Transmitter + FPV Tracking System v3.0 (Адаптивное управление с автоопределением)")
        ...
        self.throttle_pid = PIDController(0.35, 0.001, 0.001)
        self.vertical_pid = PIDController(0.3, 0.001, 0.001)  # Для вертикальной позиции
        #self.pid_x = PIDController(0.25, 0.01, 0.002)      # Для YAW
        self.pid_x = YawPidController(0.15, 0.001, 0.003, integral_limit=30, output_limit=60)
        self.pid_rx = PIDController(0.25, 0.01, 0.002)    # Для ROLL
        
        # ==================== АВТООПРЕДЕЛЕНИЕ ТИПА БАТАРЕИ ====================
        ...
        # ==================== АДАПТИВНЫЕ ПАРАМЕТРЫ ДЛЯ РАЗНЫХ РАЗМЕРОВ КВАДРОКОПТЕРОВ ====================
        ...
        self.current_profile = '5inch'
        self.profile_auto_detected = False
        
        # ==================== АДАПТИВНЫЙ УПРАВЛЯЮЩИЙ АЛГОРИТМ ====================
        ...
        print("  Адаптивное управление с автоопределением батареи")
        print("=" * 80)
    ...
    def draw_animated_selection(self, surface, x, y, width, height, animation_counter):
        """Рисует анимированную рамку выделения"""
        corner_size = 20
    ...
    def handle_mouse_selection(self, event):
        """Обработка выделения объекта мышью"""
    ...
    def update_adaptive_throttle_pid(self, error_y, current_voltage, dt):
        ...
        pid_output = self.vertical_pid.update(0, error_y, dt)
    ...
    def tracking_task(self):
        ...
        while not self.tracking_stop_event.is_set() and self.running:
            try:
                ...
                try:
                    object_size = self.current_tracker_bbox[2] if self.current_tracker_bbox else 100
                    object_aspect = self.current_tracker_bbox[3] / self.current_tracker_bbox[2] if self.current_tracker_bbox and self.current_tracker_bbox[2] > 0 else 1.0
                except:
                    object_size = 100
                    object_aspect = 1.0
    ...
    def update_joystick_values(self):
        """Обновление значений джойстика с адаптивным PID для throttle"""
        ...
        with self.tracking_data_lock:
            ...
            object_size = self.tracking_data.get("object_size", 100)
        
        if not self.interactive_mode and self.tracking_active and confidence > 0.3:
            current_time = time.time()
            dt = current_time - self.last_update
            dt = max(0.001, min(0.1, dt))
            self.last_update = current_time
            
            # ==================== НОВАЯ ЛОГИКА: ЛЕТИТЬ К ЦЕЛИ ====================
            # 1. ГОРИЗОНТАЛЬНОЕ ПОЛОЖЕНИЕ (error_x) → YAW
            pid_x = self.pid_x.update(0, error_x, dt)
            pid_x = max(-20, min(20, pid_x))
            
            # 2. ВЕРТИКАЛЬНОЕ ПОЛОЖЕНИЕ (error_y) → THROTTLE
            pid_y = self.vertical_pid.update(0, -error_y, dt)  # Инвертируем знак
            pid_y = max(-100, min(100, pid_y))
            
            # 3. ФИКСИРОВАННЫЙ PITCH 8% ВПЕРЕД (ИЗМЕНЕНИЕ!)
            # Вместо PID для расстояния - устанавливаем фиксированный наклон 8% вперед
            fixed_pitch_percent = 0.24  # 8% вперед
            SCALE = 127 / 100.0
            ...
            raw_pitch = max(max_pitch_backward, min(max_pitch_forward, raw_pitch))
            
            # 4. ГОРИЗОНТАЛЬНАЯ СТАБИЛИЗАЦИЯ → ROLL (оставляем нейтральным или для компенсации)
            pid_roll = 0  # Нейтральное значение или небольшая компенсация
            
            # ==================== ПРЕОБРАЗОВАНИЕ В ДИАПАЗОН 0-255 ====================
            SCALE = 127 / 100.0
            
            # YAW: 128 ± pid_x * SCALE (центр 128)
            raw_yaw = 128 + pid_x * SCALE
            
            # THROTTLE: базовый газ + вертикальная коррекция
            profile = self.quadcopter_profiles[self.current_profile]
            base_throttle = profile['hover_throttle']
            ...
            if voltage > 0:
                ...
                throttle_output = base_throttle + pid_y * profile['pid_scale']
            
            raw_throttle = max(50, min(220, throttle_output))
            
            # PITCH уже вычислен как фиксированный
            raw_roll = 128 + pid_roll * SCALE
            raw_roll = max(100, min(156, raw_roll))  # Небольшой диапазон для компенсации
            
            # ==================== АДАПТИВНЫЕ КОЭФФИЦИЕНТЫ ====================
            # Увеличиваем агрессивность при высокой уверенности
            if confidence > 0.7:
                self.pid_x.kp = 0.3
                self.vertical_pid.kp = 0.4
            elif confidence > 0.4:
                self.pid_x.kp = 0.2
                self.vertical_pid.kp = 0.3
            else:
                self.pid_x.kp = 0.15
                self.vertical_pid.kp = 0.2
            
            # ==================== ПРИМЕНЕНИЕ ЕВРО-ФИЛЬТРА ====================
            if self.euro_filter_enabled:
                filtered_yaw = self.one_euro_filter('yaw_auto', raw_yaw, current_time)
                filtered_throttle = self.one_euro_filter('throttle_auto', raw_throttle, current_time)
                filtered_pitch = self.one_euro_filter('pitch_auto', raw_pitch, current_time)
                filtered_roll = self.one_euro_filter('roll_auto', raw_roll, current_time)
                
                self.x = max(0, min(255, int(round(filtered_yaw))))
                self.y = max(0, min(255, int(round(filtered_throttle))))
                self.ry = max(0, min(255, int(round(filtered_pitch))))
                self.rx = max(0, min(255, int(round(filtered_roll))))
                
                #self.display_values['filter_type'] = f"АВТО (Евро+Фиксированный Pitch 8%)"
            else:
                smooth_factor = 0.7
                
                self.x = int(self.x + (raw_yaw - self.x) * smooth_factor)
                self.y = int(self.y + (raw_throttle - self.y) * smooth_factor)
                self.ry = int(self.ry + (raw_pitch - self.ry) * smooth_factor)
                self.rx = int(self.rx + (raw_roll - self.rx) * smooth_factor)
                ...
                #self.display_values['filter_type'] = f"АВТО (Простой+Фиксированный Pitch 8%)"
            
            # ==================== ОБНОВЛЕНИЕ ДЛЯ ОТОБРАЖЕНИЯ ====================
            # Расчет процентов для отображения
            yaw_percent = int(((self.x - 128) / 127) * 100)
            throttle_percent = int((self.y / 255) * 100)
            pitch_percent = int(((self.ry - 128) / 127) * 100)
            roll_percent = int(((self.rx - 128) / 127) * 100)
            
            # Определение направления движения
            direction = f"▶ Pitch фиксирован: {pitch_percent}% вперед"
            direction_color = (0, 200, 255)  # Голубой для фиксированного pitch
            ...
            self.display_values.update({
                ...
                'fps': fps,
                'confidence': confidence,
                'throttle_pct': throttle_percent,
                'pitch_percent': pitch_percent,
                'roll_percent': roll_percent,
                'yaw_percent': yaw_percent,
                'direction': direction,
                'direction_color': direction_color,
                'battery_info': f"{self.battery_analyzer['battery_type']} {self.battery_analyzer['detected_cells']}S",
                'quad_profile': self.current_profile,
            })
            
            # Статус трекинга с направлением
            if confidence > 0.7:
                status_text = f"ТРЕКИНГ: Pitch фиксирован {pitch_percent}%"
                self.display_values['tracking_status'] = status_text
                self.display_values['tracking_color'] = direction_color
            elif confidence > 0.4:
                self.display_values['tracking_status'] = f"СЛЕЖЕНИЕ: Pitch фиксирован {pitch_percent}%"
                self.display_values['tracking_color'] = (255, 165, 0)
            else:
                self.display_values['tracking_status'] = "ПОИСК"
                self.display_values['tracking_color'] = (255, 0, 0)
        
        elif not self.tracking_active and not self.interactive_mode:
            # При паузе плавно возвращаем стики в нейтраль
            smooth_factor = 0.1
            
            # Мягкий возврат к нейтрали
            self.x = int(self.x + (128 - self.x) * smooth_factor)
            self.rx = int(self.rx + (128 - self.rx) * smooth_factor)
            self.ry = int(self.ry + (128 - self.ry) * smooth_factor)
            
            # Газ остается на текущем уровне
            current_throttle = self.y
            # Мягкое снижение газа, но не до нуля
            target_throttle = max(50, current_throttle * 0.95)
            self.y = int(self.y + (target_throttle - self.y) * smooth_factor)
            
            self.display_values['tracking_status'] = "ПАУЗА"
            self.display_values['tracking_color'] = (255, 255, 0)
            self.display_values['throttle_pct'] = int((self.y / 255) * 100)
            self.display_values['direction'] = "⏸ ПАУЗА"
            self.display_values['direction_color'] = (255, 255, 0)
    ...
    def handle_interactive_input(self):
        ...
        # Получаем данные о напряжении для коррекции
        try:
            voltage = float(self.telemetry_parser.telemetry['voltage'])
            if voltage > 0 and self.battery_analyzer['auto_detection_complete']:
                cells = self.battery_analyzer['detected_cells']
                nominal_voltage = self.battery_analyzer['nominal_voltage_per_cell'] * cells
                if voltage < nominal_voltage * 0.85:
                    # Коррекция газа при низком напряжении
                    voltage_factor = voltage / (nominal_voltage * 0.9)
                    self._throttle_current *= voltage_factor
        except:
            pass
        ...
        # Адаптивные клавиши управления
        if keys[pygame.K_p] and not getattr(self, '_p_pressed', False):
            self._p_pressed = True
            profiles = list(self.quadcopter_profiles.keys())
            current_idx = profiles.index(self.current_profile)
            next_idx = (current_idx + 1) % len(profiles)
            self.current_profile = profiles[next_idx]
            self.profile_auto_detected = False
            self.active_buttons.add("P")
        elif not keys[pygame.K_p]:
            self._p_pressed = False
        
        if keys[pygame.K_o] and not getattr(self, '_o_pressed', False):
            self._o_pressed = True
            if self.battery_analyzer['auto_detection_complete']:
                self.auto_detect_quadcopter_profile()
            self.active_buttons.add("O")
        elif not keys[pygame.K_o]:
            self._o_pressed = False
        
        if keys[pygame.K_i] and not getattr(self, '_i_pressed', False):
            self._i_pressed = True
            self.adaptive_control['learning_enabled'] = not self.adaptive_control['learning_enabled']
            status = "ВКЛ" if self.adaptive_control['learning_enabled'] else "ВЫКЛ"
            self.active_buttons.add("I")
    ...
    def create_gui_elements(self):
        ...
        # Панель адаптивного управления
        self.adaptive_rect = pygame.Rect(720, 20, 450, 100)
    ...
    def draw_interface(self):
        ...
        if has_recent_telemetry:
            # Battery info
            voltage_str = self.telemetry_parser.telemetry['voltage']
            current_str = self.telemetry_parser.telemetry['current']
            fuel_str = self.telemetry_parser.telemetry['fuel']
            
            voltage_color = (0, 255, 0) if voltage_str != "N/A" else (255, 165, 0)
            
            batt_text = self.small_font.render(f"Voltage: {voltage_str} V", True, voltage_color)
            self.screen.blit(batt_text, (self.telemetry_rect.x + 10, y_pos))
            y_pos += 20
            
            current_text = self.small_font.render(f"Current: {current_str} A", True, voltage_color)
            self.screen.blit(current_text, (self.telemetry_rect.x + 10, y_pos))
            y_pos += 20
            
            fuel_text = self.small_font.render(f"Remaining: {fuel_str}", True, voltage_color)
        ...
        info_items = [
            ...
            (f"Tracking: {self.display_values['tracking_status']}", 
             self.display_values.get('tracking_color', (255, 255, 255))),
            ...
            (f"Pitch: {self.display_values.get('pitch_percent', 0)}%", (0, 200, 255)),
            ...
            #(f"Профиль: {self.display_values.get('quad_profile', '5inch')}", (200, 200, 255)),
            #(f"Батарея: {self.display_values.get('battery_info', 'Unknown')}", (200, 200, 255)),
            #(f"Направление: {self.display_values.get('direction', 'N/A')}", 
            #self.display_values.get('direction_color', (255, 255, 255))),
        ]
        ...
        # === Pitch индикатор ===
        pitch_rect = pygame.Rect(self.screen_width - 160, 450, 120, 20)
        pitch_fill = pygame.Rect(self.screen_width - 160, 450, 
                                int(((self.ry - 128) / 127) * 60 + 60), 20)
        
        pygame.draw.rect(self.screen, (40, 40, 40), pitch_rect, border_radius=3)
        
        # Цвет в зависимости от направления
        if self.ry > 128:
            pitch_color = (0, 200, 255)  # Голубой для pitch вперед
        else:
            pitch_color = (255, 100, 0)  # Оранжевый для pitch назад
            
        pygame.draw.rect(self.screen, pitch_color, pitch_fill, border_radius=3)
        pygame.draw.rect(self.screen, self.BLACK, pitch_rect, 2, border_radius=3)
        
        pitch_text = self.font.render(f"Pitch: {self.display_values.get('pitch_percent', 0)}%", 
                                     True, pitch_color)
        self.screen.blit(pitch_text, (self.screen_width - 155, 420))
        ...
        controls = [
            ...
            "I/K - PITCH (правый стик Y) - возвращается в центр (только в ручном режиме)",
            ...
            "ПРИМЕЧАНИЕ: При трекинге PITCH фиксирован",
            "ESC - Выход"
        ]
        ...
        for i, control in enumerate(controls):
            if i == 0:
                ...
                color = (0, 255, 255)
            elif "ПРИМЕЧАНИЕ" in control:  # Выделяем примечание о pitch
                color = (0, 200, 255)  # Голубой для примечания
            else:
                color = (200, 200, 255)
    ...
    def run(self):
        ...
        print("=" * 80)
        print("Starting Combined FPV System with Adaptive Control")
        print("  Auto-detection of battery type and quadcopter profile")
        print("  Fixed Pitch 8% forward during tracking")
        print("  Works with any quadcopter from TinyWhoop to X-Class")
        print("=" * 80)
        print("Controls:")
        print("  TAB - Toggle interactive mode")
        print("  M - Switch between LOCAL/ELRS modes")
        print("  F - Toggle Euro filter")
        print("  P - Change quadcopter profile")
        print("  O - Force auto-detection")
        print("  I - Toggle adaptive learning")
        print("  LEFT MOUSE BUTTON - Select object for tracking")
        print("  NOTE: Pitch fixed at N% forward during tracking")
        print("  ESC - Exit")
        print("=" * 80)
