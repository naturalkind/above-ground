...
# Версия с интегрированным сбором данных
...
# ============================================================================
# СБОРЩИК ДАННЫХ С НУЛЕВЫМИ ПОТЕРЯМИ
# ============================================================================

class ZeroLossDataCollector:
    """
    Сборщик данных с нулевым влиянием на производительность
    """
    def __init__(self, system_instance, enabled=True):
        self.system = system_instance
        self.enabled = enabled
        self.recording = False
        self.frame_counter = 0
        
        # Буферы в RAM (циклические)
        self.frame_buffer = collections.deque(maxlen=100)  # Последние 100 кадров
        self.data_buffer = collections.deque(maxlen=1000)  # 1000 записей данных
        
        # Статистика (нулевой оверхед)
        self.stats = {
            'total_frames': 0,
            'tracked_frames': 0,
            'avg_error_x': 0.0,
        ...
        # Предварительно выделенные массивы для статистики
        self.error_buffer_x = np.zeros(100, dtype=np.float32)
        self.error_buffer_y = np.zeros(100, dtype=np.float32)
        self.buffer_idx = 0
        
        # Флаг высокочастотного сбора
        self.high_freq_collect = False
        self.collect_every_n = 1  # Собирать каждый кадр
        
        print(f"Zero-loss data collector initialized (enabled: {enabled})")
    
    def collect_data(self, frame=None):
        """
        Сбор данных с минимальным оверхедом
        Возвращает True если данные собраны
        """
        ...
        # Пропускаем кадры если не high_freq режим
        if not self.high_freq_collect and self.frame_counter % self.collect_every_n != 0:
            return False
        
        try:
            # Сбор минимальных данных (O(1) операции)
            tracking_data = {
                'timestamp': time.perf_counter(),
                'frame_id': self.frame_counter,
                'error_x': self.system.display_values.get('error_x', 0),
                'error_y': self.system.display_values.get('error_y', 0),
                'confidence': self.system.display_values.get('confidence', 0),
                'fps': self.system.display_values.get('fps', 0),
                'tracking_active': self.system.tracking_active,
                'bbox': self.system.tracker_lib.bbox if hasattr(self.system, 'tracker_lib') else [0,0,0,0]
            }
            ...
            joystick_data = {
                ...
                'rx': self.system.rx,
                'ry': self.system.ry,
                'throttle_pct': self.system.display_values.get('throttle_pct', 0),
                'aux1': self.system.aux1_state,
                'aux2': self.system.aux2_state
            }
            
            pid_data = {
                'pid_x': self.system.display_values.get('pid_x', 0),
                'pid_y': self.system.display_values.get('pid_y', 0),
                'pid_x_kp': self.system.pid_x.kp if hasattr(self.system, 'pid_x') else 0,
                'pid_x_ki': self.system.pid_x.ki if hasattr(self.system, 'pid_x') else 0,
                'pid_x_kd': self.system.pid_x.kd if hasattr(self.system, 'pid_x') else 0,
                'pid_y_kp': self.system.pid_y.kp if hasattr(self.system, 'pid_y') else 0,
                'pid_y_ki': self.system.pid_y.ki if hasattr(self.system, 'pid_y') else 0,
                'pid_y_kd': self.system.pid_y.kd if hasattr(self.system, 'pid_y') else 0
            }
            
            # Сохранение в буфер (неблокирующее)
            data_packet = {
            ...
            self._update_stats(tracking_data['error_x'], tracking_data['error_y'])
            
            # Сохранение кадра если есть
            if frame is not None and len(self.frame_buffer) < self.frame_buffer.maxlen:
                self.frame_buffer.append(frame.copy())
            
            return True
            
        except Exception as e:
            # Тихий сбой - не мешаем основной работе
            return False
    
    def _update_stats(self, error_x, error_y):
        """Быстрое обновление статистики"""
        # Обновляем циклический буфер
        if self.buffer_idx < 100:
            self.error_buffer_x[self.buffer_idx] = error_x
            self.error_buffer_y[self.buffer_idx] = error_y
            self.buffer_idx += 1
        else:
            # Сдвигаем буфер
            self.error_buffer_x[:-1] = self.error_buffer_x[1:]
            self.error_buffer_y[:-1] = self.error_buffer_y[1:]
            self.error_buffer_x[-1] = error_x
            self.error_buffer_y[-1] = error_y
        
        # Обновляем статистику
        self.stats['total_frames'] = self.frame_counter
        self.stats['last_error_x'] = error_x
        self.stats['last_error_y'] = error_y
        
        # Периодический пересчет средней ошибки
        if self.frame_counter % 30 == 0:
            valid_data = self.buffer_idx if self.buffer_idx < 100 else 100
            if valid_data > 0:
                self.stats['avg_error_x'] = np.mean(self.error_buffer_x[:valid_data])
                self.stats['avg_error_y'] = np.mean(self.error_buffer_y[:valid_data])
    
    def start_recording(self, high_freq=False):
        ...
        self.recording = True
        self.frame_counter = 0
        self.high_freq_collect = high_freq
        self.collect_every_n = 1 if high_freq else 3  # Каждый кадр или каждый 3-й
        ...
        print(f"Data recording started (high_freq: {high_freq})")
        return True
    ...
    def stop_recording(self):
        ...
        # Сохранение данных на диск (в отдельном потоке)
        threading.Thread(
            target=self._save_data_to_disk,
            daemon=True
        ).start()
        
        print(f"Data recording stopped. Collected {self.frame_counter} frames")
        return True
    
    def _save_data_to_disk(self):
        """Сохранение данных на диск (в фоне)"""
        try:
            ...
            save_dir = f"data_collection/session_{timestamp}"
            os.makedirs(save_dir, exist_ok=True)
            
            # Сохранение данных
            data_to_save = list(self.data_buffer)
            
            # 1. Сохранение как pickle (быстро)
            with open(f"{save_dir}/data.pkl", 'wb') as f:
                pickle.dump(data_to_save, f)
            
            # 2. Сохранение как CSV (для анализа)
            self._save_as_csv(data_to_save, f"{save_dir}/data.csv")
            
            # 3. Сохранение кадров (если есть)
            if self.frame_buffer:
                self._save_frames(f"{save_dir}/frames")
            ...
            metadata = {
                ...
                'duration_estimated': self.frame_counter / 30,  # Предполагаем 30 FPS
                'stats': self.stats,
                'high_freq': self.high_freq_collect
            }
            ...
            print(f"  - {len(data_to_save)} data points")
            print(f"  - {len(self.frame_buffer)} frames")
            print(f"  - Avg error X: {self.stats['avg_error_x']:.2f}, Y: {self.stats['avg_error_y']:.2f}")
    ...
    def _save_as_csv(self, data, filename):
        ...
        try:
            with open(filename, 'w', newline='') as csvfile:
                ...
                # Заголовки
                headers = [
                    'frame_id', 'timestamp', 'error_x', 'error_y', 'confidence',
                    'fps', 'tracking_active',
                    'joystick_x', 'joystick_y', 'joystick_rx', 'joystick_ry',
                    'throttle_pct', 'aux1', 'aux2',
                    'pid_x', 'pid_y'
                ]
    ...
    def _save_frames(self, dir_path):
        """Сохранение кадров"""
        try:
            ...
            for i, frame in enumerate(self.frame_buffer):
                if frame is not None:
                    filename = f"{dir_path}/frame_{i:04d}.jpg"
...
class CombinedFPVSystem:
    def __init__(self, screen_width=1800, screen_height=1000):
        ...
        self.selection_animation_speed = 0.2
        ...
        # ==================== СБОРЩИК ДАННЫХ ====================
        self.data_collector = ZeroLossDataCollector(self, enabled=True)
        self.data_collection_active = False
        self.data_collector_gui_elements_created = False
        ...
        self.create_data_collector_gui_elements()
        
        print("=" * 80)
        print("CombinedFPVSystem инициализирован с архитектурой:")
        print("  Трекер (CV) → Калман (позиция) → PID → Евро-фильтр → Джойстик")
        print("  Zero-loss data collector integrated")
        print("=" * 80)
    
    # ==================== МЕТОДЫ СБОРЩИКА ДАННЫХ ====================
    
    def create_data_collector_gui_elements(self):
        """Создание GUI элементов для управления сбором данных"""
        self.data_collector_gui_elements_created = True
        
        # Кнопки управления сбором данных
        self.data_record_button = pygame.Rect(780, 70, 150, 40)
        self.data_status_indicator = pygame.Rect(950, 70, 150, 40)
        self.data_stats_display = pygame.Rect(1120, 70, 200, 40)
        
        # Панель статистики данных
        self.data_stats_panel = pygame.Rect(20, 820, 350, 170)
    
    def draw_data_collector_interface(self):
        """Отрисовка интерфейса сборщика данных"""
        if not self.data_collector_gui_elements_created:
            return
        
        # Панель статистики данных
        pygame.draw.rect(self.screen, (40, 40, 60), self.data_stats_panel, border_radius=8)
        pygame.draw.rect(self.screen, (80, 80, 100), self.data_stats_panel, 2, border_radius=8)
        
        data_title = self.font.render("Data Collection", True, (200, 200, 255))
        self.screen.blit(data_title, (self.data_stats_panel.x + (self.data_stats_panel.width - data_title.get_width()) // 2, 
                                     self.data_stats_panel.y + 10))
        
        y_pos = self.data_stats_panel.y + 40
        
        if self.data_collector.enabled:
            stats = self.data_collector.get_status()
            
            # Статус записи
            status_color = (255, 50, 50) if stats['recording'] else (100, 100, 100)
            status_text = "RECORDING" if stats['recording'] else "READY"
            status_surface = self.small_font.render(f"Status: {status_text}", True, status_color)
            self.screen.blit(status_surface, (self.data_stats_panel.x + 20, y_pos))
            y_pos += 20
            
            # Счетчик кадров
            frames_text = f"Frames: {stats['frame_counter']}"
            frames_surface = self.small_font.render(frames_text, True, (200, 200, 255))
            self.screen.blit(frames_surface, (self.data_stats_panel.x + 20, y_pos))
            y_pos += 20
            
            # Размер буфера
            buffer_text = f"Buffer: {stats['buffer_size']}"
            buffer_surface = self.small_font.render(buffer_text, True, (200, 200, 255))
            self.screen.blit(buffer_surface, (self.data_stats_panel.x + 20, y_pos))
            y_pos += 20
            
            # Средние ошибки
            avg_error_text = f"Avg error X: {stats['stats']['avg_error_x']:.1f}, Y: {stats['stats']['avg_error_y']:.1f}"
            avg_error_surface = self.small_font.render(avg_error_text, True, (200, 200, 255))
            self.screen.blit(avg_error_surface, (self.data_stats_panel.x + 20, y_pos))
            y_pos += 20
            
            # Частота сбора
            freq_text = f"Mode: {'High' if stats['high_freq'] else 'Low'} freq"
            freq_surface = self.small_font.render(freq_text, True, (200, 200, 255))
            self.screen.blit(freq_surface, (self.data_stats_panel.x + 20, y_pos))
            
        else:
            disabled_text = self.small_font.render("Data collector disabled", True, (150, 150, 150))
            self.screen.blit(disabled_text, (self.data_stats_panel.x + (self.data_stats_panel.width - disabled_text.get_width()) // 2, 
                                           self.data_stats_panel.y + 60))
        
        # Кнопка записи данных
        if self.data_collector.enabled:
            collector = self.data_collector
            record_color = (255, 50, 50) if collector.recording else (50, 255, 50)
            pygame.draw.rect(self.screen, record_color, self.data_record_button, border_radius=5)
            pygame.draw.rect(self.screen, self.BLACK, self.data_record_button, 2, border_radius=5)
            record_text = self.font.render("REC" if collector.recording else "START", True, self.WHITE)
            self.screen.blit(record_text, 
                           (self.data_record_button.x + (self.data_record_button.width - record_text.get_width()) // 2,
                            self.data_record_button.y + 10))
        
        # Индикатор статуса
        indicator_color = (255, 0, 0) if self.data_collector.enabled and self.data_collector.recording else (100, 100, 100)
        pygame.draw.rect(self.screen, indicator_color, self.data_status_indicator, border_radius=5)
        pygame.draw.rect(self.screen, self.BLACK, self.data_status_indicator, 2, border_radius=5)
        
        if self.data_collector.enabled:
            frames_text = self.font.render(f"F: {self.data_collector.frame_counter}", True, self.WHITE)
            self.screen.blit(frames_text, 
                           (self.data_status_indicator.x + (self.data_status_indicator.width - frames_text.get_width()) // 2,
                            self.data_status_indicator.y + 10))
    
    def handle_data_collector_events(self, event):
        """Обработка событий сборщика данных"""
        if event.type == pygame.MOUSEBUTTONDOWN:
            pos = event.pos
            
            # Кнопка записи данных
            if self.data_collector.enabled and self.data_record_button.collidepoint(pos):
                if self.data_collector.recording:
                    self.data_collector.stop_recording()
                    print("Data recording stopped")
                else:
                    # Запрашиваем режим сбора
                    self.data_collector.start_recording(high_freq=False)  # Low freq по умолчанию
                    print("Data recording started (low frequency)")
                return True
            
            # Кнопка статуса (двойной клик для high freq)
            if (self.data_collector.enabled and 
                self.data_status_indicator.collidepoint(pos) and 
                event.button == 3):  # Правая кнопка
                
                if not self.data_collector.recording:
                    self.data_collector.start_recording(high_freq=True)
                    print("Data recording started (high frequency)")
                return True
        
        elif event.type == pygame.KEYDOWN:
            # Горячие клавиши для управления сбором данных
            if event.key == pygame.K_F8:
                # Включение/выключение сборщика
                self.data_collector.enabled = not self.data_collector.enabled
                status = "enabled" if self.data_collector.enabled else "disabled"
                ...
                if self.data_collector.recording:
                    self.data_collector.stop_recording()
                    print("Data recording stopped (F9)")
                else:
                    self.data_collector.start_recording(high_freq=False)
                    print("Data recording started (F9)")
                return True
            
            elif event.key == pygame.K_F10:
                # High frequency запись
                if not self.data_collector.recording:
                    self.data_collector.start_recording(high_freq=True)
                    print("High frequency data recording started (F10)")
                return True
        
        return False
    
    def collect_current_data(self):
        """Сбор текущих данных системы"""
        if not self.data_collector.enabled or not self.data_collector.recording:
            return
        
        # Получаем текущий кадр если доступен
        current_frame = None
        if hasattr(self, 'camera_surface') and self.camera_surface:
            # Конвертируем Pygame surface в numpy array
            try:
                # Получаем пиксели из surface
                pixel_string = pygame.image.tostring(self.camera_surface, 'RGB')
                
                # Создаем numpy массив
                img_array = np.frombuffer(pixel_string, dtype=np.uint8)
                
                # Решейпим в изображение
                width, height = self.camera_surface.get_width(), self.camera_surface.get_height()
                img_array = img_array.reshape((height, width, 3))
                
                # Конвертируем RGB в BGR (OpenCV формат)
                current_frame = cv2.cvtColor(img_array, cv2.COLOR_RGB2BGR)
            except:
                current_frame = None
        
        # Сбор данных через сборщик
        self.data_collector.collect_data(current_frame)
    ...
    def draw_interface(self):
        ...
        controls = [
            ...
            "F8 - Вкл/Выкл сбор данных",
            "F9 - Старт/стоп записи данных",
            "F10 - High frequency запись данных",
            "ЛЕВАЯ КНОПКА МЫШИ - Выделить объект для трекинга",
            "ПРАВАЯ КНОПКА МЫШИ - High freq запись",
            "ESC - Выход"
        ]
        ...
        for i, control in enumerate(controls):
            if i == 0:
                ...
                color = (0, 255, 255)
            elif "F8" in control or "F9" in control or "F10" in control:  # Горячие клавиши данных
                color = (255, 200, 0)  # Золотистый для горячих клавиш данных
            else:
                color = (200, 200, 255)
        ...
        self.draw_data_collector_interface()
    ...
    def handle_events(self):
        ...
        for event in pygame.event.get():
            ...
            # Обработка событий сборщика данных
            if self.handle_data_collector_events(event):
                continue
    ...
    def run(self):
        ...
        print("  F8 - Enable/disable data collector")
        print("  F9 - Start/stop data recording (low frequency)")
        print("  F10 - Start high frequency recording")
        print("  Right mouse on status indicator - Start high frequency")
        print("=" * 80)
    ...
    def cleanup(self):
        ...
        # Остановка сбора данных если активен
        if self.data_collector and self.data_collector.recording:
            self.data_collector.stop_recording()
