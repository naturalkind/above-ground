...
#                parsed = {'type': 'BARO', 'altitude': altitude}
            elif packet_type == CRSF_FRAMETYPE_BARO_ALTITUDE and len(payload) >= 2:
                # Baro altitude: uint16_t, decimeters with 10000 offset
                altitude_packed = struct.unpack('>H', payload[0:2])[0]  # Big-endian
                
                # Декодирование: вычитаем смещение 10000 дециметров
                altitude_decimeters = altitude_packed - 10000
                
                # Конвертируем дециметры в сантиметры (умножаем на 10)
                altitude_centimeters = altitude_decimeters * 10
                
                # Также можно получить в метрах для сравнения с Configurator
                altitude_meters = altitude_centimeters / 100.0
                
                parsed = {
                    'type': 'BARO',
                    'altitude_cm': altitude_centimeters,  # Высота в сантиметрах
                    'altitude_m': altitude_meters,        # Высота в метрах (как в Configurator)
                    'raw_packed': altitude_packed
                }

#            elif packet_type == CRSF_FRAMETYPE_BARO_ALTITUDE and len(payload) >= 2:
#                # Baro altitude: uint16_t, meters with 1000m offset
#                altitude_packed = struct.unpack('>H', payload[0:2])[0]  # Big-endian
#                
#                # Декодирование: вычитаем смещение 1000 метров
#                altitude_meters = altitude_packed - 1000
#                
#                parsed = {
#                    'type': 'BARO',
